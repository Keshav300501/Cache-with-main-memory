...
class Memory:
	...
	def printMemory(self):
		...
		#print("The memory at location "+str(self.memoryAddress)+" is: "+str(self.memoryVal))
...
class MainMemory:
	...
	def isInMainMemory(self,memoryObj,blockTag):
		# pdb.set_trace()
		if(len(self.mainMemoryDict) == 0):
			return False

		found = False
		tmpVal = 0
		for val in self.mainMemoryDict:
			if(val == blockTag):
				found = True
				tmpVal = val
				break
		...
		if(found == True):
			if(self.mainMemoryDict[val].isInBlock(memoryObj) == True):
				return True
			else:
				return False
		else:
	...
	def addToMainMemory(self,memoryObj,blockTag):
		
		if(blockTag in self.mainMemoryDict.keys()):
			self.mainMemoryDict[blockTag].insertInMemoryArray(memoryObj)
		else:
			blockObj = Block(blockTag)
			blockObj.insertInMemoryArray(memoryObj)
			self.mainMemoryDict[blockTag] = blockObj

	def getFromMainMemory(self,blockTag):
		for val in self.mainMemoryDict:
			if(val == blockTag):
				return self.mainMemoryDict[val]

	def updateInMainMemory(self,memoryObj,blockTag):
		self.mainMemoryDict[blockTag].updateInMemoryArray(memoryObj)

	def printMainMemory(self):
		print()
		print()
		print("printing main memory")
		print()
		for val in self.mainMemoryDict:
			self.mainMemoryDict[val].printBlock()
			print()
			

MainMemoryArr = MainMemory()
TimeChecking = []
...
class FullyAssosiative:
	...
	def writeToCache(self,memoryObj,blockTag):
		...
		if(len(self.cache) == 0):
			...
			cprint("Cache tag miss","yellow")
			found = MainMemoryArr.isInMainMemory(memoryObj,blockTag)
			
			if(found == True):
				tmpBlock = MainMemoryArr.getFromMainMemory(blockTag)
		...
		if(MainMemoryArr.isInMainMemory(memoryObj,blockTag) == True):
			MainMemoryArr.updateInMainMemory(memoryObj,blockTag)
		else:
			MainMemoryArr.addToMainMemory(memoryObj,blockTag)
	...
	def readCache(self,memoryObj,blockTag):
		if(len(self.cache) == 0):
			...
			return
		# pdb.set_trace()
		if(MainMemoryArr.isInMainMemory(memoryObj,blockTag) == False):
			cprint("The memory address doesn't exits","yellow")
			return
		...
		if(found == True):
			...
			updateInTimeCheckingArr(blockTag)
			MainMemoryArr.updateInMainMemory(tmpMemoryObj,blockTag)


		else:
			cprint("Cache tag miss","yellow")
			tmpObj = MainMemoryArr.getFromMainMemory(blockTag)
			tmpMemoryObj = tmpObj.getFromBlock(memoryObj)

			if(self.crntCacheSize < self.cacheLine):
				self.crntCacheSize += 1
				self.cache[blockTag] = tmpObj
			...
			TimeChecking.append(blockTag)
			MainMemoryArr.updateInMainMemory(tmpMemoryObj,blockTag)
			print("the value at the address "+str(tmpMemoryObj.memoryAddress)+" is "+str(tmpMemoryObj.memoryVal))
			#print(TimeChecking)
...
class DirectMapping:
	...
	def writeToCache(self,memoryObj,blockTag):
		...
		if(len(self.cache) == 0):
			...
			if(MainMemoryArr.isInMainMemory(memoryObj,blockTag) == True):
				tmpBlockObj = MainMemoryArr.getFromMainMemory(blockTag)
				tmpBlockObj.updateInMemoryArray(memoryObj)
				...
				if(self.isIndexInCache(dictIndex) == True):
					...
					self.cache[dictIndex] = tmpBlockObj
	...
	def readCache(self,memoryObj,blockTag):
		...
		if(MainMemoryArr.isInMainMemory(memoryObj,blockTag) == False):
			print("The memory address doesn't exits")
			return
		...
		if(self.isIndexInCache(dictIndex) == True):
			if(self.cache[dictIndex].blockTag == blockTag):
				...
				cprint("Cache Miss","yellow")
				tmpObj = MainMemoryArr.getFromMainMemory(blockTag)
				tmpMemoryObj = tmpObj.getFromBlock(memoryObj)
				self.cache[dictIndex] = tmpObj
				print("The value at the address is "+str(tmpMemoryObj.memoryVal))
...
class SetObject:
	def __init__(self,cacheLines,blockSize):
		...
		self.crntCacheSize = 0
		self.setCache ={}
		global globalTime,MainMemoryArr
	...
	def writeInSetCache(self,memoryObj,blockTag):
		...
		if(len(self.setCache) == 0):
			...
			if(found == True):
				...
				if(self.crntCacheSize<self.cacheLines):
					...
					for val in self.setCache:
						if(val == tmpPoppedBlockTag):
							deleteBlock = self.setCache.pop(val)
							break
					self.setCache[blockTag] = tmpBlock
					self.LocalTimeChecking.append(blockTag)
				return
		...
		if(MainMemoryArr.isInMainMemory(memoryObj,blockTag)):
			MainMemoryArr.updateInMainMemory(memoryObj,blockTag)
		else:
			MainMemoryArr.addToMainMemory(memoryObj,blockTag)
	...
	def readFromSetCache(self,memoryObj,blockTag):
		if(MainMemoryArr.isInMainMemory(memoryObj,blockTag) == False):
			cprint("Memory address doesn't exists","yellow")
			return
